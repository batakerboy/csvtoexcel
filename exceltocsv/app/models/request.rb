	...
	def self.import(file=nil)
		if !file.nil?
			...
			spreadsheet.each_with_index do |row, i|
				...
				@request.ut_time = token[6] unless token[5] == 'Sunday'
				@request.vacation_leave = token[7] unless token[5] == 'Sunday'
				@request.sick_leave = token[8] unless token[5] == 'Sunday'
				@request.official_business = token[9] unless token[5] == 'Sunday'
